...
import { contact, trainingGroups } from '@/data/content';
...
export function LearningPaths() {
  ...
  const group = trainingGroups[selected];
...
export function ContactForm() {
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const message = `Olá, Guto! Sou ${fields.get('name')}, da empresa ${fields.get('company') || 'não informada'}. Quero conversar sobre ${fields.get('solution')}.\n\n${fields.get('message')}`;
    window.open(`https://wa.me/5554981302517?text=${encodeURIComponent(message)}`,'_blank','noopener,noreferrer');
  }
  return <form className="contact-form" onSubmit={submit}><div className="form-row"><div><label htmlFor="contact-name">Seu nome</label><input id="contact-name" name="name" autoComplete="name" placeholder="Como podemos chamar você?" required maxLength={100}/></div><div><label htmlFor="contact-company">Empresa</label><input id="contact-company" name="company" autoComplete="organization" placeholder="Nome da empresa" maxLength={150}/></div></div><label htmlFor="contact-solution">Sobre o que vamos conversar?</label><select id="contact-solution" name="solution" defaultValue="um projeto digital"><option value="um projeto digital">Quero entender as possibilidades</option><option>Treinamentos e trilhas de conhecimento</option><option>Gamificação</option><option>Projetos especiais 3D</option><option>Plataformas inteligentes</option><option>Experiências imersivas</option><option>Soluções para RH</option></select><label htmlFor="contact-message">Conte um pouco sobre o desafio</label><textarea id="contact-message" name="message" rows={4} placeholder="O que você gostaria de transformar na sua empresa?" required maxLength={2500}/><button className="action" type="submit">Conversar no WhatsApp <ArrowUpRight size={19}/></button><p className="form-note">Atendimento com {contact.name}. Seus dados não são armazenados neste formulário.</p></form>;
}
